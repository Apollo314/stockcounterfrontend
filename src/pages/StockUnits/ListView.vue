...
import type { StockUnit } from 'src/client';

type Row = BaseRow & StockUnit;

type Column = BaseColumn<Row>;

const operation = get_operation('/inventory/stock_unit/', 'get');
  ...
  [
    {
      callback: (rows, done) => {
        router.push({
          ...
          params: { name: Array.from(rows)[0][1].name },
        });
        done(false);
      },
...
function fetcher({
  ...
  api.inventory
    .inventoryStockUnitList(queryMaker(requestPagination))
    .then((value) => {
